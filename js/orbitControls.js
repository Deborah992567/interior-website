/**
 * OrbitControls — Three.js r128 compatible (standalone, no module system)
...
(function () {
  ...
  THREE.OrbitControls = function (object, domElement) {
    ...
    function handleMouseWheel(event) {
      if (event.deltaY < 0) { dollyIn(getZoomScale()); } else if (event.deltaY > 0) { dollyOut(getZoomScale()); }
      scope.update();
    }
    ...
    function onPointerDown(event) {
      ...
      switch (event.button) {
        case 0:
          if (scope.mouseButtons.LEFT === THREE.MOUSE.ROTATE) { handleMouseDownRotate(event); state = STATE.ROTATE; }
          else if (scope.mouseButtons.LEFT === THREE.MOUSE.PAN) { handleMouseDownPan(event); state = STATE.PAN; }
          break;
        case 1: handleMouseDownDolly(event); state = STATE.DOLLY; break;
        case 2:
          if (scope.mouseButtons.RIGHT === THREE.MOUSE.PAN) { handleMouseDownPan(event); state = STATE.PAN; }
          else if (scope.mouseButtons.RIGHT === THREE.MOUSE.ROTATE) { handleMouseDownRotate(event); state = STATE.ROTATE; }
          break;
      }
      if (state !== STATE.NONE) {
    ...
    function onTouchStart(event) {
      ...
      switch (event.touches.length) {
        case 1: if (scope.touches.ONE === THREE.TOUCH.ROTATE) { handleTouchStartRotate(event); state = STATE.TOUCH_ROTATE; } break;
      }
    ...
    function onTouchMove(event) {
      ...
      event.preventDefault();
      switch (state) {
        case STATE.TOUCH_ROTATE: if (scope.enableRotate) handleTouchMoveRotate(event); break;
      }
    }
